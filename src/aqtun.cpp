...
func s_tile_visual get_tile_index(s_map* map, int x, int y)
{
	...
	bool top_left_check     = do_x_check && map->entity_arr[y    ][x - 1].type == e_entity_wall;
	bool top_right_check    = map->entity_arr[y    ][x    ].type == e_entity_wall;
	bool bottom_right_check = do_y_check && map->entity_arr[y - 1][x    ].type == e_entity_wall;
	bool bottom_left_check  = (do_x_check && do_y_check) && map->entity_arr[y - 1][x - 1].type == e_entity_wall;
